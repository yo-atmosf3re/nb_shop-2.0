...
const FullPageItems: React.FC<FullPageItemsPropsType> = ({ count, id, image, price, title }) => {
   ...
   return (
      <>
         <div className="fullpage__image-large">
            <img src={NB574L} alt="" />
         </div>
         <div className="fullpage__sneakers-title">
            {title}
         </div>
         <div className="fullpage__items-model">
            Item model number: MT91547
         </div>
         <div className='fullpage__info'>
            <div onClick={addItemCart} className="info__shoppingcart">
            ...
            <div className="info__price">$ {divedesNumbers(Number(price))}</div>
         </div>
      </>
   )
}
